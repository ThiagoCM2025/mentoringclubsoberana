...
const AdminEnrollments = () => {
  ...
  const createEnrollment = async () => {
    if (!newEnrollment.userId || !newEnrollment.courseId) {
      toast({ title: "Selecione aluno e curso", variant: "destructive" });
      return;
    }
    ...
    const { data: existing } = await supabase
      .from("enrollments")
      .select("id")
      .eq("user_id", newEnrollment.userId)
      .eq("course_id", newEnrollment.courseId)
      .maybeSingle();

    if (existing) {
      toast({ title: "Aluno já está matriculado neste curso", variant: "destructive" });
      return;
    }
  ...
  const deleteEnrollment = async (id: string) => {
    ...
    if (error) {
      ...
      toast({ title: "Matrícula excluída" });
      fetchData();
    }
  };
            ...
            <div className="space-y-4 pt-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setNewEnrollment({ ...newEnrollment, userId: value })}
                >
                  <SelectTrigger>
