...
const StudentDashboard = () => {
  ...
  const fetchDailyChallengesCount = async () => {
    const {
      count
    } = await supabase.from("daily_challenges").select("*", {
      count: "exact",
      head: true
    }).eq("is_active", true).eq("challenge_type", "daily");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (enrollmentData) {
        ...
        const normalized = (enrollmentData as unknown as EnrollmentWithCourse[]).filter(e => Boolean(e.courses));
  ...
  const fetchCourseProgress = async (courseId: string) => {
    ...
    const {
      data: progressData
    } = await supabase.from("progress").select("lesson_id, completed").eq("user_id", user.id).in("lesson_id", lessonIds);
  ...
  const fetchContinueWatching = async (enrollments: EnrollmentWithCourse[]) => {
    ...
    for (const enrollment of enrollments) {
      ...
      const {
        data: lessons
      } = await supabase.from("lessons").select("id, title, duration_minutes").in("module_id", moduleIds).order("order_index");
      if (!lessons || lessons.length === 0) continue;
      const lessonIds = lessons.map(l => l.id);
      const {
        data: progressData
      } = await supabase.from("progress").select("lesson_id, progress_seconds, completed").eq("user_id", user.id).in("lesson_id", lessonIds);
      for (const lesson of lessons) {
        ...
        if (lessonProgress && !lessonProgress.completed && lessonProgress.progress_seconds > 0) {
          ...
          const progressPercent = durationSeconds > 0 ? Math.round(lessonProgress.progress_seconds / durationSeconds * 100) : 0;
          items.push({
            lessonId: lesson.id,
  ...
  return <div className="min-h-screen bg-black">
      ...
      y: 0
        ...
        <div className="container-soberana flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Button variant="ghost" size="sm" onClick={() => navigate("/student")} className="relative text-secondary font-medium hover:bg-secondary/10 group">
              Início
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4/5 h-0.5 bg-secondary rounded-full" />
            </Button>
            <Button variant="ghost" size="sm" onClick={() => navigate("/student/favorites")} className="relative text-cream/70 hover:text-secondary hover:bg-secondary/10 group">
              Favoritos
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate("/student/community")} className="relative text-cream/70 hover:text-secondary hover:bg-secondary/10 group">
              Comunidade
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-secondary rounded-full transition-all duration-300 group-hover:w-4/5" />
            </Button>
          </nav>
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  opacity: 1,
                  scale: 1
                }} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-secondary/10 border border-secondary/30 cursor-pointer hover:bg-secondary/20 transition-colors ${showPulse ? 'animate-pulse-subtle' : ''}`} onClick={() => navigate("/student/achievements")}>
                    <Star className="w-3.5 h-3.5 text-secondary" />
                    <span className="text-xs font-medium text-cream">{gamificationStats?.xp || 0}</span>
                    {showPulse && <span className="relative flex h-2 w-2">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  delay: 0.1
                }} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-500/10 border border-orange-500/30 cursor-pointer hover:bg-orange-500/20 transition-colors ${showPulse ? 'animate-pulse-subtle' : ''}`} onClick={() => navigate("/student/achievements")}>
                ...
                <TooltipContent side="bottom" className="bg-zinc-900 border-secondary/30 text-cream">
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-xs text-cream/70">
                      {gamificationStats?.streak_days === 0 ? "Comece a estudar hoje!" : gamificationStats?.streak_days === 1 ? "1 dia consecutivo" : `${gamificationStats?.streak_days} dias consecutivos`}
                    </p>
            ...
            <Button variant="ghost" size="icon" className="text-cream/70 hover:text-secondary hover:bg-secondary/10">
              <Search className="w-5 h-5" />
            </Button>
            <StudyReminderButton />
            <NotificationBell />
            <Button variant="ghost" size="icon" className="text-cream/70 hover:text-secondary hover:bg-secondary/10" onClick={() => navigate("/student/profile")}>
              <User className="w-5 h-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={handleSignOut} className="text-cream/70 hover:text-red-400 hover:bg-red-400/10">
              <LogOut className="w-5 h-5" />
            </Button>
          </div>

          {/* Mobile Menu Button */}
          <Button variant="ghost" size="icon" className="md:hidden text-cream" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
      ...
      y: 0
          ...
          opacity: 1,
          scale: 1
        }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary/10 border border-secondary/30 cursor-pointer hover:bg-secondary/20 transition-colors ${showPulse ? 'animate-pulse-subtle' : ''}`} onClick={() => {
          ...
          delay: 0.1
        }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/30 cursor-pointer hover:bg-orange-500/20 transition-colors ${showPulse ? 'animate-pulse-subtle' : ''}`} onClick={() => {
          navigate("/student/achievements");
          setMobileMenuOpen(false);
        }}>
          ...
          setMobileMenuOpen(false);
              ...
              <Button variant="ghost" className="justify-start text-cream/70 hover:text-secondary hover:bg-secondary/10" onClick={() => {
          ...
          setMobileMenuOpen(false);
              ...
              <Button variant="ghost" onClick={handleSignOut} className="justify-start text-cream/70 hover:text-red-400 hover:bg-red-400/10">
      ...
      <section className="relative h-[420px] md:h-[480px] w-full overflow-hidden">
        {/* Background Image */}
        <img src={heroVariations} alt="Hero" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(15)].map((_, i) => <motion.div key={i} className="absolute w-1 h-1 rounded-full bg-secondary/50" initial={{
          x: `${Math.random() * 100}%`,
          y: "110%",
          opacity: 0
        }} animate={{
        ...
        <div className="relative z-10 h-full flex items-center container-soberana px-4">
          ...
          duration: 0.6
            ...
            {/* Welcome text */}
            <h1 className="text-2xl md:text-3xl font-serif text-cream mb-4 leading-tight">Bem-vinda ao Mentoring Club Soberana </h1>
      ...
      <main className="container-soberana py-8 px-4">
        ...
        <section className="mb-12">
          ...
          <motion.div className="grid md:grid-cols-3 gap-6" initial="hidden" animate="visible" variants={{
          hidden: {
            opacity: 0
          },
          visible: {
            ...
            transition: {
              ...
              <div className="aspect-[4/3] relative">
                {welcomeVideoUrl ? <>
                  ...
                  </> : <img src={heroVariations} alt="Plataforma" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />}
          ...
          }} onClick={() => navigate("/student/community")} className="relative group bg-zinc-900 rounded-xl border border-secondary/20 overflow-hidden hover:border-secondary/50 transition-all cursor-pointer hover:shadow-lg">
              <div className="aspect-[4/3] relative bg-gradient-to-br from-zinc-800 to-black flex flex-col items-center justify-center">
                <motion.div className="w-16 h-16 rounded-full bg-secondary/20 border border-secondary/40 flex items-center justify-center mb-4" whileHover={{
                scale: 1.1,
                rotate: 5
              }} transition={{
                type: "spring",
          ...
          }} onClick={() => navigate("/student/achievements")} className="relative group bg-zinc-900 rounded-xl border border-secondary/20 overflow-hidden hover:border-secondary/50 transition-all cursor-pointer hover:shadow-lg">
              <div className="aspect-[4/3] relative bg-gradient-to-br from-zinc-800 to-black flex flex-col items-center justify-center">
                <motion.div className="w-16 h-16 rounded-full bg-secondary/20 border border-secondary/40 flex items-center justify-center mb-4" whileHover={{
                scale: 1.1,
                rotate: -5
        ...
        y: 0
          ...
          scale: 0.98
        }} onClick={() => navigate("/student/achievements")} className="relative cursor-pointer bg-gradient-to-br from-zinc-900 via-black to-zinc-900 rounded-2xl p-6 border border-secondary/30 overflow-hidden group">
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  ...
                  {showPulse && <span className="absolute -top-1 -right-1 w-4 h-4 bg-orange-500 rounded-full animate-ping" />}
                </div>
              ...
              <div className="flex items-center gap-2 text-secondary">
                {dailyChallengesCount > 0 && <span className="bg-orange-500 text-white text-xs font-bold px-2 py-1 rounded-full">
        ...
        <section className="mb-12">
          ...
          delay: 0.3
            ...
            <a href="/#jornada" target="_blank" rel="noopener noreferrer">
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6" initial="hidden" animate="visible" variants={{
          hidden: {
            opacity: 0
          },
          visible: {
            ...
            return <ProgramCard key={program.slug} program={program} index={index} isEnrolled={isEnrolled} courseId={courseId} />;
          })}
          </motion.div>
        </section>
        ...
        <section className="mb-12">
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4" initial="hidden" animate="visible" variants={{
          hidden: {
            opacity: 0
          },
          visible: {
            ...
            {/* WhatsApp Comunidade */}
            <motion.a href="https://wa.me/5511993563468?text=Olá! Sou aluna da plataforma Soberana e gostaria de entrar no grupo da comunidade." target="_blank" rel="noopener noreferrer" variants={{
            hidden: {
              opacity: 0,
              y: 20,
              scale: 0.9
            ...
            {/* Agendar Mentoria */}
            <motion.a href="https://wa.me/5511993563468?text=Olá Fabiana! Sou aluna da plataforma e gostaria de agendar uma mentoria individual." target="_blank" rel="noopener noreferrer" variants={{
            hidden: {
              opacity: 0,
              y: 20,
              scale: 0.9
            ...
            scale: 0.98
          }} onClick={() => navigate("/student/favorites")} className="group relative p-5 bg-zinc-900 rounded-xl border border-secondary/20 hover:border-secondary/50 transition-all cursor-pointer overflow-hidden hover:shadow-lg">
              <div className="absolute inset-0 bg-gradient-to-br from-secondary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className="relative z-10">
                <div className="w-12 h-12 rounded-full bg-secondary/20 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
            ...
            scale: 0.98
          }} onClick={() => navigate("/student/certificates")} className="group relative p-5 bg-zinc-900 rounded-xl border border-secondary/20 hover:border-secondary/50 transition-all cursor-pointer overflow-hidden hover:shadow-lg">
              <div className="absolute inset-0 bg-gradient-to-br from-secondary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className="relative z-10">
                <div className="w-12 h-12 rounded-full bg-secondary/20 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
        ...
        {/* Stats com Stagger */}
        <motion.div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10" initial="hidden" animate="visible" variants={{
        hidden: {
          opacity: 0
        },
        visible: {
          ...
          visible: {
            ...
            <StatsCard icon={BookOpen} label="Cursos Ativos" value={enrollments.length} color="primary" index={0} />
          </motion.div>
          <motion.div variants={{
          hidden: {
            opacity: 0,
          ...
          visible: {
            ...
            <StatsCard icon={PlayCircle} label="Aulas Concluídas" value={totalCompleted} color="green" index={1} />
          </motion.div>
          <motion.div variants={{
          hidden: {
            opacity: 0,
          ...
          visible: {
            ...
            <StatsCard icon={Clock} label="Horas de Estudo" value={Math.round(totalCompleted * 0.25)} color="secondary" index={2} />
          </motion.div>
          <motion.div variants={{
          hidden: {
            opacity: 0,
          ...
          visible: {
            ...
            <StatsCard icon={Award} label="Certificados" value={Object.values(progress).filter(p => p === 100).length} color="accent" index={3} />
          </motion.div>
        </motion.div>
        ...
        <section className="py-8 text-center border-t border-secondary/10 mt-8">
          ...
          <Button variant="ghost" size="sm" className="text-secondary hover:bg-secondary/10" onClick={() => window.open("https://wa.me/5511993563468?text=Olá! Preciso de ajuda com a plataforma Soberana.", "_blank")}>
      ...
      <CoursePreviewModal courseId={previewCourseId} isOpen={!!previewCourseId} onClose={() => setPreviewCourseId(null)} />
      ...
      <BadgeCelebrationModal badge={newBadge} isOpen={showCelebration} onClose={closeCelebration} />

      {/* Welcome Video Modal */}
      <WelcomeVideoModal open={showWelcomeVideo} onOpenChange={setShowWelcomeVideo} videoUrl={welcomeVideoUrl} />
    </div>;
};
